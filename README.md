...
**Pages:** `/` (homepage, includes the Story section), `/campus`, `/co-living`, `/education`, `/events`, `/library`, `/political-club`, `/people`, `/the-protocol`. **Education** is the internal hub for [Fractal Accelerator](https://www.fractalaccelerator.com/) and [FractalU](https://www.fractalu.nyc/); its cards link to each live site in a new tab. (Old paths redirect internally: `/story`→`/`, `/visit`→`/co-living`, `/publications`→`/library`, `/neighborhood`→`/co-living`, `/lab`→`/library`, and both `/accelerator` and `/new-liberal-arts`→`/education`.)
