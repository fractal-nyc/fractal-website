...
**Pages:** `/` (homepage, includes the Story section), `/campus`, `/co-living`, `/education`, `/events`, `/library`, `/political-club`, `/people`, `/the-protocol`. Unlisted Campus member guide: `/members` (intended for `https://members.fractalnyc.com`; `/members/guide` redirects here) — not in the public navbar; see [`docs/members-home.md`](./docs/members-home.md). **Education** is the first-party Fractal University home: its public catalog and information portal live natively on `/education` in a Library-style archive. Accelerator data and redirects remain available for its future dedicated destination, but the page has no standalone Accelerator promotion. (Old paths redirect internally: `/story`→`/`, `/visit`→`/co-living`, `/publications`→`/library`, `/neighborhood`→`/co-living`, `/lab`→`/library`, and both `/accelerator` and `/new-liberal-arts`→`/education`.)
