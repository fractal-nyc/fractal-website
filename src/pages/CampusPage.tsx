    ...
    >
      ...
      <div className="relative z-10">
        ...
        <div
          ref={bannerRef}
          aria-hidden="true"
          className="pointer-events-none fixed inset-x-4 sm:inset-x-8 md:inset-x-12 lg:inset-x-16 top-28 lg:top-36 z-0 hidden lg:flex justify-between"
        ...
        {/* Mobile-only flanking pennants — bold moment before the footer.
            Desktop uses the absolute `hidden lg:flex` layer above; this block is
            `flex lg:hidden` and sits in normal flow. Each pennant gets an explicit
            aspect-ratio (SVG viewBox ~123:368) so `h-full w-full` on the inner
            <img> resolves without a fixed-height ancestor. */}
        <div
          aria-hidden="true"
          className="flex lg:hidden items-end justify-center gap-3 px-3 pt-8 pb-12"
        >
