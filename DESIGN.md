...
The Home “Curious about Fractal?” label uses the People accent directly, connecting that invitation to the People section. `#C49040` on cream is 6.33:1 against charcoal but only 2.62:1 when used as the foreground itself, so this small gold label is an **operator-approved decorative text exception**. It must not be used for body copy, controls, or essential instructions; those remain charcoal on cream.
