...
- **`FractalUniversityPortal`** — a native `bg-background text-foreground` editorial surface inside the deep Education page. It owns the static public catalog snapshot, responsive source-owned collage, 44px subject filters with pressed state and one polite result announcement, stacked course cards below `lg`, a bounded desktop table at `lg+`, keyboard-operable description/bio disclosures, four group cards, and the public FractalU information/canon. Every nested surface reasserts its text pairing; HTTP actions explicitly open a safe new tab. It is never an iframe, runtime scrape, authentication surface, or separate route.
