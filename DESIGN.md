...
`.text-aside` is the editorial italic voice — bylines, attributions, role labels, parenthetical asides. It pairs with `.text-subtitle` on quoted passages: the quote uses `.text-subtitle`, the byline uses `.text-aside`. Article Card bylines specifically use this Inter role; Course Card instructor names use the upright Inter `.text-body` role. Card categories and compact facts remain mono chrome through `.text-label`.
