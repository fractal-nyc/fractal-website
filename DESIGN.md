...
(**FractalU and Accelerator:** *Education* (`school`, shown in the nav as *FractalU*) has an internal, education-colored page at `/fractalu`; its catalog uses a cream inner surface for dense, readable course data. `/education` and `/new-liberal-arts` redirect there. *Accelerator* remains external at `https://www.fractalaccelerator.com/`. Both keep their house token pairs, nav-letter color, and octahedron vertex node. Still **7 houses / 14 tokens**.)
