...
(**Education hub:** Accelerator is a program within the Education house, not a seventh house. `/education` is the first-party decision point for Accelerator and FractalU: a Display statement and explanatory subtitle introduce two non-interactive program containers, each with a subtitle-tier name, stable body copy, and a standalone Campus-style CTA that opens the official site in a new tab. No external site is embedded. The navbar has one internal Education link. The octahedron retains separate `Accelerator` and `FractalU` labels and vertices, but both route internally to `/education`; route-driven rotation deliberately uses Accelerator as the representative pose while both nodes share the active glow. Legacy `/accelerator` and `/new-liberal-arts` paths redirect internally to the hub.)
...
**Scope:** house colors live on their own house's pages. On a house's page, its pair may color display headings, the monogram letter, eyebrows, focus rings, and chrome. Most houses use `{light}` as the page background and `{deep}` as the accent; **Political Club and Education invert** (page background `{deep}`, accent `{light}`) — per-page decisions applied in `PoliticalClubPage.tsx` and `EducationPage.tsx`. Education's program containers stay on the deep page surface with restrained light-red border/frost chrome; cream text carries their heading and body copy. Per-page banner SVG artwork is migrated separately; until that artwork PR lands, the embedded SVG fills may temporarily retain the previous palette even though page and component tokens use this canonical table.
