...
(**Education hub:** Accelerator is a program within the Education house, not a seventh house. `/education` is the first-party decision point for Fractal AI Accelerator and Fractal University: the Display statement “A new liberal arts” and an explanatory subtitle introduce two equal-height, whole-card external links to the canonical program sites. No external site is embedded. The navbar has one internal Education link. The octahedron retains separate `Accelerator` and `FractalU` labels and vertices, but both route internally to `/education`; route-driven rotation deliberately uses Accelerator as the representative pose while both nodes share the active glow. Legacy `/accelerator` and `/new-liberal-arts` paths redirect internally to the hub.)
...
**Scope:** house colors live on their own house's pages. On a house's page, its pair may color display headings, the monogram letter, eyebrows, focus rings, and chrome. Most houses use `{light}` as the page background and `{deep}` as the accent; **Political Club and Education invert** (page background `{deep}`, accent `{light}`) — per-page decisions applied in `PoliticalClubPage.tsx` and `EducationPage.tsx`. Education nests warm light program cards inside its deep-red page: the resting cards pair charcoal copy with Education-light red titles, borders, and decorations, then invert to Education-light red with cream copy and decorations on hover or keyboard focus. Per-page banner SVG artwork is migrated separately; until that artwork PR lands, the embedded SVG fills may temporarily retain the previous palette even though page and component tokens use this canonical table.
