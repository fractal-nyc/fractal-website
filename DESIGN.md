...
**Scope:** house colors live on their own house's pages. On a house's page, its pair may color display headings, the monogram letter, eyebrows, focus rings, and chrome. Most houses use `{light}` as the page background and `{deep}` as the accent; **Political Club and Education invert** (page background `{deep}`, accent `{light}`) — per-page decisions applied in `PoliticalClubPage.tsx` and `EducationPage.tsx`. Education's external Accelerator card rests on a warm cream/frost surface and inverts to `{light}` with cream text on hover/focus. Its native Fractal University portal is a canonical `bg-background text-foreground` editorial surface nested within the deep Education page; Education red is reserved for controls, category labels, focus, and card chrome. Per-page banner SVG artwork is migrated separately; until that artwork PR lands, the embedded SVG fills may temporarily retain the previous palette even though page and component tokens use this canonical table.
...
Education program cards have one additional operator-approved, decorative-only use of `section-people`: a restrained tint mixed into the canonical cream resting surface. This surface mixture adds parchment warmth but does **not** make People a third Education identity color. People gold is never used for Education text, borders, controls, or essential information; the card's identity and interaction states remain entirely Education red, cream, and charcoal.
...
- **Education Accelerator card** — the sole program-level external card on `/education`. It is a whole-card link with 30px Mandelbrot corners and at least `p-9`, rests on a warm cream/frost surface with charcoal body text and Education-light mono label/chrome, and inverts to `house-education-light` with cream text on hover/focus. It always names the new tab and uses the tracked Accelerator URL from `src/data/education.ts`; reduced motion removes scale/translation while preserving the color affordance.
- **`FractalUniversityPortal`** — a native portal inside the deep Education page. Its opaque `bg-background text-foreground` shell is a `relative z-20`, page-guttered, Library-scale foreground capped at 1600px so the single course-card collection covers rather than competes with the fixed desktop pennants. It owns the static public snapshot, responsive source-owned collage, 44px subject filters with pressed state and one polite result announcement, one content-driven `auto-fit/minmax` card grid at every width, and four group cards. Category is each course card's sole uppercase mono eyebrow; normal-case titles link to the independently stored verified course document (with one source-accurate unlinked fallback), while application/video actions remain distinct. Descriptions and all source-verified instructor biographies stay visible in normal flow on touch, narrow, and large-text layouts; at `64rem+` on hover-capable fine pointers they become hover/focus previews, with instructor buttons supporting pin/unpin and Escape. Multi-instructor courses preserve each exact biography as a separate ordered prose record inside one preview panel. A near-top fragment link scrolls and focuses the bottom `What is FractalU?` section. That information, etiquette, and canon return to the deep surface using Campus's `max-w-7xl` frame and `max-w-3xl` reading column; “Want to teach?” uses the Visit/Home-Story `MandelbrotCorners size="sm"` cream callout with `p-9`. Every nested surface reasserts its text pairing; HTTP actions explicitly open a safe new tab. It is never an iframe, runtime scrape, authentication surface, or separate route.
...
- **Education program card** — the two canonical programs render in source order as one column at the 375px baseline and equal-height columns at `md`. Each program is a single whole-card external anchor: no nested or standalone CTA button. Its warm resting fill is a token-based mixture of canonical cream and a restrained `section-people` decorative tint, paired explicitly with charcoal body copy and Education-light red border, mono-label title, four `MandelbrotCorners size="sm"` glyphs, accent rule, and upper-corner `ArrowUpRight`. The 30px glyphs sit at 6px insets, so card padding remains `p-9` (36px) on every breakpoint. Hover and keyboard focus invert the surface to Education-light red with cream copy/decorations; the Library/Visit vocabulary supplies the 200ms ease-out scale/lift and shadow, arrow nudge/fade, and 300ms rule expansion. Focus-visible carries a real 2px cream ring. Under `prefers-reduced-motion`, color changes remain immediate while transitions, scale, and arrow translation are disabled.
