...
**Scope:** house colors live on their own house's pages. On a house's page, its pair may color display headings, the monogram letter, eyebrows, focus rings, and chrome. Most houses use `{light}` as the page background and `{deep}` as the accent; **Political Club and Education invert** (page background `{deep}`, accent `{light}`) — per-page decisions applied in `PoliticalClubPage.tsx` and `EducationPage.tsx`. Education's cream destination-card surfaces explicitly pair with charcoal text; its light red is decorative border/chrome rather than essential small text. Per-page banner SVG artwork is migrated separately; until that artwork PR lands, the embedded SVG fills may temporarily retain the previous palette even though page and component tokens use this canonical table.
