...
- **`FractalUniversityPortal`** — a native portal inside the deep Education page. Fractal University owns the page `h1`, subtitle, future-semester action, and accessible in-page information jump. Its transparent `relative z-20`, page-guttered Library-scale catalog is capped at 1600px so cream course/group cards float over the deep field while banner fragments remain visible through the gaps. The archive hierarchy is semester heading, support line, divider, 44px subject filters with pressed state, divider, and one polite result announcement. One content-driven `auto-fit/minmax` grid holds the courses and four group cards follow. Every card uses 20px Mandelbrot corners and Library's 200ms lift/border/shadow response; reduced motion removes transforms while preserving the border and color affordance. Category is each course card's sole uppercase mono eyebrow; normal-case titles link to the independently stored verified course document (with one source-accurate unlinked fallback), while application/video actions remain distinct. Linked title arrows are visible on touch and reveal/scale on fine-pointer hover and keyboard focus. Descriptions and all source-verified instructor biographies stay visible in normal flow on touch, narrow, and large-text layouts; at `64rem+` on hover-capable fine pointers they become hover/focus previews, with instructor buttons supporting pin/unpin and Escape. Multi-instructor courses preserve each exact biography as a separate ordered prose record inside one preview panel. The information, etiquette, and canon use Campus's `max-w-7xl` frame and `max-w-3xl` reading column; “Want to teach?” uses the Visit/Home-Story `MandelbrotCorners size="sm"` cream callout with `p-9`. The uncropped source collage appears once as the final meaningful content section. Every nested surface reasserts its text pairing; HTTP actions explicitly open a safe new tab. It is never an iframe, runtime scrape, authentication surface, or separate route.
