...
To refresh a semester: compare both canonical pages; update only publicly rendered course/group fields and links in `fractalu-catalog.json`; update semester and `FRACTALU_SNAPSHOT_DATE`; replace the two collage files only when the source assets changed and preserve their intrinsic dimensions; check every outbound label/href; run the portal, Education, navigation, and house-token tests; then visually verify filters, the one responsive card collection, pointer previews, touch-visible descriptions/bios, and no horizontal overflow. Each course title reads its external course-document destination from that record's independently editable optional `detailsUrl`; never substitute an application/calendar URL for a missing document link. Descriptions and available instructor bios are inline content on touch/narrow/large-text layouts and become hover/focus previews only on wide fine-pointer layouts. Missing links and bios remain explicit source-accurate fallbacks—do not invent them. Do not add private fields, organizer/session metadata that is not visible, authentication, a `/fractalu` route, or a second FractalU program card.
