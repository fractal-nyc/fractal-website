...
import { ComponentColorScope, COMPONENT_COLORWAYS, getComponentSurfaceTone, type ComponentColorwayId, type ComponentSurfaceMode } from "@/components/content/ComponentColorScope";
...
import { CourseCardCollection, ClubCard, CourseSubjectFilter } from "@/components/education/FractalUniversityPortal";
...
const courseSubjectOptions = [
  ...
  { value: "Experimental category", label: "Experimental category (generic icon)" },
] as const;
...
const COMPONENT_REGISTRY_BASE: ComponentRegistryEntry[] = [
  ...
  defineSpecimen({
    id: "outbound-link", name: "Standalone Link", componentName: "OutboundLink", category: "Actions & controls", sourcePath: "src/components/content/OutboundLink.tsx", purpose: "A prominent link that stands on its own and uses the diagonal outbound arrow.", useWhen: "Use for a visible destination link that sits outside a sentence.", doNotUseWhen: "Do not use inside prose or for a linked card title.", contentFields: ["link label", "destination"], variants: ["Website", "Email"], themeable: true, surfaceModes: ["paper", "light", "deep"], defaultColorway: "neutral", defaultSurface: "paper", accessibility: "HTTP links open safely in a new tab and announce that behavior when an accessible name is supplied; email links stay in context.", responsive: "Long labels wrap without losing the arrow or target size.", agentPhrase: "Use the **Standalone Link** component.", promptNeeds: "link label and destination", usedOn: "Education course, club, and canon destinations, plus Events/Luma.", keywords: ["external link", "outbound link", "outsource link", "external action link", "arrow link", "url"], aliases: ["External Link", "Outbound Link", "outsource link"],
    ...
    render: ({ colorway, surface, values }) => { const href = values.destination === "email" ? "mailto:hello@fractal.nyc" : "https://fractal.nyc"; return <div className="flex items-start"><OutboundLink href={href} accessibleName={values.label} tone={getComponentSurfaceTone(colorway, surface)} surfaceAdaptive>{values.label}</OutboundLink></div>; },
  }),
  defineSpecimen({
    id: "outbound-text-link", name: "Outbound Text Link", componentName: "OutboundLink", category: "Actions & controls", sourcePath: "src/components/content/OutboundLink.tsx", purpose: "An Inter text link with an arrow that sits outside a sentence and inherits the surrounding body or lead size.", useWhen: "Use for a text destination that sits outside a sentence and needs an arrow.", doNotUseWhen: "Do not use inside a sentence, for compact mono actions, or to choose a larger text size.", contentFields: ["link label", "destination", "surrounding text context"], variants: ["Body text", "Large / lead text", "Website", "Email"], themeable: true, surfaceModes: ["paper", "deep"], defaultColorway: "neutral", defaultSurface: "paper", accessibility: "Uses native link semantics, a 44px target, visible focus, and safe new-tab behavior for HTTP destinations.", responsive: "Inherits its containing Inter type size; long labels wrap while the arrow stays attached to the destination text.", agentPhrase: "Use the **Outbound Text Link** component.", promptNeeds: "link label and destination; inherit the surrounding Inter body or lead text context", usedOn: "Education header destinations in a lead-text context.", keywords: ["inter outbound link", "large text link", "education hero link", "prominent link", "text link with arrow", "body outbound link"], aliases: ["Prominent Text Link", "Inter Outbound Link", "Large Text Link", "Education Hero Link"],
    controls: [textControl("label", "Link label", "Stay tuned for future semesters", "Explore the complete semester program"), selectControl("context", "Example text context", "body", "lead", [{ value: "body", label: "Body text" }, { value: "lead", label: "Large / lead text" }]), selectControl("destination", "Destination type", "external", "email", [{ value: "external", label: "External website" }, { value: "email", label: "Email" }]), PREVIEW_WIDTH_CONTROL, COLORWAY_CONTROL, DEEP_SURFACE_CONTROL],
    render: ({ colorway, surface, values }) => { const href = values.destination === "email" ? "mailto:fractalu@fractalnyc.com" : "https://fractaluniversity.substack.com"; return <div className={`${values.context === "lead" ? "text-body-lead" : "text-body"} flex items-start`} data-text-link-context={values.context}><OutboundLink href={href} accessibleName={values.label} variant="outbound" tone={getComponentSurfaceTone(colorway, surface)} surfaceAdaptive>{values.label}</OutboundLink></div>; },
  }),
  defineSpecimen({
    id: "inline-text-link", name: "Inline Text Link", componentName: "OutboundLink", category: "Actions & controls", sourcePath: "src/components/content/OutboundLink.tsx", purpose: "Underlined Inter words inside a sentence, without an arrow, inheriting the surrounding body or lead size.", useWhen: "Use for linked words that are part of surrounding prose.", doNotUseWhen: "Do not use for a standalone destination, linked card title, or an entire clickable card.", contentFields: ["linked words", "destination", "surrounding sentence", "surrounding text context"], variants: ["Body text", "Large / lead text", "Website", "Email"], themeable: true, surfaceModes: ["paper", "light", "deep"], defaultColorway: "neutral", defaultSurface: "paper", accessibility: "Uses native link semantics and visible focus; website destinations retain safe new-tab behavior while email links stay in context.", responsive: "Inherits its containing Inter type size and wraps naturally with the surrounding sentence.", agentPhrase: "Use the **Inline Text Link** component.", promptNeeds: "linked words and destination; inherit the surrounding Inter body or lead text context", usedOn: "Campus contact copy, Co-Living and Home / Story prose, and the Education teaching note.", keywords: ["inline link", "inline reference", "text link", "prose link", "link in a sentence"], aliases: ["Inline Link", "Inline Reference"],
    controls: [textControl("label", "Linked words", "fractalu@fractalnyc.com", "crystal@fractalnyc.com"), selectControl("context", "Example text context", "body", "lead", [{ value: "body", label: "Body text" }, { value: "lead", label: "Large / lead text" }]), selectControl("destination", "Destination type", "email", "external", [{ value: "external", label: "External website" }, { value: "email", label: "Email" }]), PREVIEW_WIDTH_CONTROL, COLORWAY_CONTROL, DEEP_SURFACE_CONTROL],
    render: ({ colorway, surface, values }) => { const href = values.destination === "email" ? `mailto:${values.label}` : "https://fractal.nyc"; const link = <OutboundLink href={href} variant="inline" tone={getComponentSurfaceTone(colorway, surface)} surfaceAdaptive>{values.label}</OutboundLink>; return values.context === "lead" ? <p className="text-body-lead" data-text-link-context="lead">First time here? Contact Crystal ({link}) for a guided tour.</p> : <p className="text-body" data-text-link-context="body">Email {link} if you want to teach at Fractal University.</p>; },
  }),
  defineSpecimen({ id: "search-bar", name: "Search Bar", componentName: "SearchBar", category: "Actions & controls", sourcePath: "src/components/content/SearchBar.tsx", purpose: "One shared search field for navigating the whole site or filtering a collection.", useWhen: "Use when someone needs to find a destination or narrow a visible collection.", doNotUseWhen: "Do not use a collection searchbox as a site-navigation combobox, or vice versa.", contentFields: ["what is searched", "mode", "selection behavior"], variants: ["Search the whole site", "Filter this collection", "Focused", "Clearable"], themeable: true, surfaceModes: ["paper", "light", "deep"], defaultColorway: "neutral", defaultSurface: "paper", accessibility: "The site mode preserves combobox/listbox keyboard behavior; collection mode is a labelled searchbox with one working Clear button.", responsive: "Fills its container with a 44px target and keeps its caret clear of end controls.", agentPhrase: "Use the **Search Bar** component.", promptNeeds: "search scope and whether choosing a result navigates or filters", usedOn: "Home site search and Library collection search.", keywords: ["search", "home search bar", "homepage search", "archive search field", "hero search", "combobox", "collection filter"], aliases: ["Home Search Bar", "Homepage Search", "Archive Search Field", "Hero Search / Combobox", "Global Search"], controls: [selectControl("mode", "Search behavior", "site", "collection", [{ value: "site", label: "Search the whole site" }, { value: "collection", label: "Filter this collection" }]), PREVIEW_WIDTH_CONTROL, COLORWAY_CONTROL, SURFACE_CONTROL], render: ({ values }) => <SearchBarSpecimen key={values.mode} mode={values.mode} /> }),
  ...
  defineSpecimen({
    id: "note-callout", name: "Note / Callout Card", componentName: "CalloutCard", category: "Cards & containers", sourcePath: "src/components/content/CalloutCard.tsx", purpose: "Highlights a short note or instruction without becoming a full content card.", useWhen: "Use for notes such as Visiting NYC?, Curious about Fractal?, or Want to teach?.", doNotUseWhen: "Do not use for long articles, repeated catalog items, or a separate action button.", contentFields: ["label", "prose", "optional inline text link", "corner size", "surface"], variants: ["Paper", "Tint", "Inline text link", "Long copy", "XS/Small/Medium/Large corners"], themeable: true, surfaceModes: ["paper", "light", "deep"], defaultColorway: "co-living", defaultSurface: "light", accessibility: "Associate the label with its containing section when it acts as a heading; links remain native prose links.", responsive: "Natural height and wrapping; no fixed width.", agentPhrase: "Use the **Note / Callout Card** component.", keywords: ["note container", "callout", "highlight", "info box"], controls: [textControl("label", "Note label", "A useful note", "Semester reminder"), selectControl("treatment", "Card treatment", "paper", "tint", [{ value: "paper", label: "Paper" }, { value: "tint", label: "Tint" }]), selectControl("body", "Body content", "standard", "long", [{ value: "standard", label: "Standard" }, { value: "long", label: "Long" }]), selectControl("cornerSize", "Corner size", "sm", "lg", [{ value: "xs", label: "XS" }, { value: "sm", label: "Small" }, { value: "md", label: "Medium" }, { value: "lg", label: "Large" }]), PREVIEW_WIDTH_CONTROL, COLORWAY_CONTROL, SURFACE_CONTROL], render: ({ values }) => <CalloutCard label={values.label} surface={values.treatment} cornerSize={values.cornerSize}><p>{values.body === "long" ? longText : <>Use this for a concise explanation. Read the <OutboundLink href="https://fractal.nyc" variant="inline">inline reference</OutboundLink> for details.</>}</p></CalloutCard>,
  }),
  defineSpecimen({ id: "course-card", name: "Course Card", componentName: "CourseCard", category: "Cards & containers", sourcePath: "src/components/education/FractalUniversityPortal.tsx", purpose: "Presents one class with an automatically category-derived subject icon, instructors, description, facts, links, and optional media.", useWhen: "Use for a semester course in the Education catalog; supply the subject/category text and let the card derive its icon.", doNotUseWhen: "Do not manually rebuild this container or choose an icon per course when refreshing a semester.", contentFields: ["title", "category (subject icon derives automatically)", "ordered instructors", "schedule", "dates", "location", "price", "description", "links", "video"], variants: ["Linked title", "Missing optional links", "Multiple instructors", "Long content", "Known subject icon", "Generic fallback icon"], themeable: true, surfaceModes: ["paper", "deep"], defaultColorway: "education", defaultSurface: "deep", accessibility: "The subject icon is decorative beside its visible label; instructor biographies support keyboard pin/Escape behavior in the catalog view.", responsive: "The subject icon remains fixed while long labels wrap; biographies are inline on touch/small screens and enhanced for fine pointers.", agentPhrase: "Use the **Course Card** component.", promptNeeds: "course content—including subject/category text; the subject icon is derived automatically from that category and must not be chosen manually", keywords: ["class container", "education course", "semester", "subject icon", "category icon"], controls: [textControl("title", "Course title", sampleCourse.title, "A Long Course on Collective Inquiry and Public Life"), selectControl("subject", "Subject and icon", sampleCourse.category, "Experimental category", courseSubjectOptions), selectControl("variant", "Content variant", "default", "multiple", [{ value: "default", label: "Default" }, { value: "long", label: "Long description" }, { value: "missing", label: "Missing optional links" }, { value: "multiple", label: "Multiple instructors" }]), PREVIEW_WIDTH_CONTROL, COLORWAY_CONTROL, DEEP_SURFACE_CONTROL], render: ({ colorway, values }) => <ComponentColorScope colorway={colorway} surface="paper" style={{ backgroundColor: "transparent" }}><CourseCardCollection courses={[{ ...sampleCourse, title: values.title, category: values.subject, description: values.variant === "long" ? longText : sampleCourse.description, instructors: values.variant === "multiple" ? [...sampleCourse.instructors, { name: "Guest Faculty", bio: "A second ordered instructor biography for the multi-instructor state." }] : sampleCourse.instructors, detailsUrl: values.variant === "missing" ? undefined : sampleCourse.detailsUrl, detailsLabel: values.variant === "missing" ? undefined : sampleCourse.detailsLabel, videoUrl: values.variant === "missing" ? undefined : sampleCourse.videoUrl, instructor: values.variant === "multiple" ? `${sampleCourse.instructor}, Guest Faculty` : sampleCourse.instructor }]} animateInitialCards={false} presentationContext="component-library" /></ComponentColorScope> }),
  ...
  reference("category-icon-label", "Category Icon Label", "CategoryIconLabel", "Cards & containers", "src/components/content/CategoryIconLabel.tsx", "Keeps the decorative category icon and visible mono label aligned across Article and Course cards."),
  ...
  defineSpecimen({ id: "education-event-callout", name: "Education Event Callout", componentName: "EducationEventCallout", category: "Cards & containers", sourcePath: "src/components/education/EducationEventCallout.tsx", purpose: "Pins a dated announcement above everything on the Education page and removes itself once the date passes.", useWhen: "Use for a single upcoming Education event such as the kickoff symposium.", doNotUseWhen: "Do not use for evergreen notes — reach for the Note / Callout Card, which has no expiry.", contentFields: ["label", "headline", "body", "action label", "action URL", "hide-after date"], variants: ["Upcoming", "Past (renders nothing)", "Long copy"], themeable: true, surfaceModes: ["paper", "light", "deep"], defaultColorway: "education", defaultSurface: "deep", accessibility: "Labelled region; the headline owns the accessible name and the action is a standard outbound link.", responsive: "Contained to the page gutter; copy wraps and never forces horizontal scroll.", agentPhrase: "Use the **Education Event Callout** component.", keywords: ["event banner", "announcement", "dated notice", "expiring callout"], controls: [textControl("headline", "Headline", EDUCATION_EVENT.headline, "Open house this Sunday"), textControl("body", "Body copy", EDUCATION_EVENT.body, longText), selectControl("state", "Event state", "upcoming", "past", [{ value: "upcoming", label: "Upcoming" }, { value: "past", label: "Past (auto-hidden)" }]), PREVIEW_WIDTH_CONTROL, COLORWAY_CONTROL, SURFACE_CONTROL], render: ({ values }) => <EducationEventCallout event={{ ...EDUCATION_EVENT, headline: values.headline, body: values.body }} now={values.state === "past" ? new Date("2026-12-01T00:00:00-05:00") : new Date("2026-09-10T00:00:00-04:00")} /> }),
  defineSpecimen({ id: "editorial-quote", name: "Editorial Quote", componentName: "EditorialQuote", category: "Cards & containers", sourcePath: "src/components/content/EditorialQuote.tsx", purpose: "Formats a substantial quotation with optional semantic citation.", useWhen: "Use for attributed editorial testimony.", doNotUseWhen: "Do not use for decorative pull text without a quotation.", contentFields: ["quote paragraphs", "citation"], variants: ["With citation", "Without citation", "Long quote"], themeable: true, surfaceModes: ["paper", "light", "deep"], defaultColorway: "campus", defaultSurface: "light", accessibility: "Uses blockquote and footer semantics.", responsive: "Indent increases only when space permits.", agentPhrase: "Use the **Editorial Quote** component.", keywords: ["blockquote", "testimonial"], controls: [textControl("quote", "Quote text", "Our relationships here matter, and we conspire to push one another’s work forward.", longText), selectControl("citation", "Citation", "with", "without", [{ value: "with", label: "With citation" }, { value: "without", label: "Without citation" }]), PREVIEW_WIDTH_CONTROL, COLORWAY_CONTROL, SURFACE_CONTROL], render: ({ values }) => <EditorialQuote citation={values.citation === "with" ? "Andrew Rose · Fractal Campus co-founder" : undefined}><p>{values.quote}</p></EditorialQuote> }),
  ...
  defineSpecimen({
    ...
    contentFields: ["photo source", "descriptive alt text", "caption"], variants: ["Previous / next", "Direct photo dots", "Keyboard navigation", "Looped sequence", "Reduced motion"], themeable: false, surfaceModes: ["paper"], defaultColorway: "neutral", defaultSurface: "paper",
...
const COMMON_IDS = new Set(["action-buttons", "outbound-link", "outbound-text-link", "inline-text-link", "library-article-card", "note-callout", "course-card", "club-card", "campus-audience-highlight"]);
...
const GALLERY_IDS = new Set([
  "action-buttons", "outbound-link", "outbound-text-link", "inline-text-link", "search-bar", "filter-bar",
