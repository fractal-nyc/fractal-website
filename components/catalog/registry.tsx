...
const COMPONENT_REGISTRY_BASE: ComponentRegistryEntry[] = [
  ...
  defineSpecimen({
    ...
    render: ({ values }) => { const href = values.destination === "email" ? `mailto:${values.label}` : "https://fractal.nyc"; const link = <OutboundLink href={href} variant="inline">{values.label}</OutboundLink>; return values.context === "lead" ? <p className="text-body-lead" data-text-link-context="lead">First time here? Contact Crystal ({link}) for a guided tour.</p> : <p className="text-body" data-text-link-context="body">Email {link} if you want to teach at Fractal University.</p>; },
  }),
  ...
  defineSpecimen({ id: "course-card", name: "Course Card", componentName: "CourseCard", category: "Cards & containers", sourcePath: "src/components/education/FractalUniversityPortal.tsx", purpose: "Presents one class with an automatically category-derived subject icon, instructors, description, facts, links, and optional media.", useWhen: "Use for a semester course in the Education catalog; supply the subject/category text and let the card derive its icon.", doNotUseWhen: "Do not manually rebuild this container or choose an icon per course when refreshing a semester.", contentFields: ["title", "category (subject icon derives automatically)", "ordered instructors", "schedule", "dates", "location", "price", "description", "links", "video"], variants: ["Linked title", "Missing optional links", "Multiple instructors", "Long content", "Known subject icon", "Generic fallback icon"], themeable: true, surfaceModes: ["paper"], accessibility: "The subject icon is decorative beside its visible label; instructor biographies support keyboard pin/Escape behavior in the catalog view.", responsive: "The subject icon remains fixed while long labels wrap; biographies are inline on touch/small screens and enhanced for fine pointers.", agentPhrase: "Use the **Course Card** component.", promptNeeds: "course content—including subject/category text; the subject icon is derived automatically from that category and must not be chosen manually", keywords: ["class container", "education course", "semester", "subject icon", "category icon"], controls: [textControl("title", "Course title", sampleCourse.title, "A Long Course on Collective Inquiry and Public Life"), selectControl("subject", "Subject and icon", sampleCourse.category, "Experimental category", courseSubjectOptions), selectControl("variant", "Content variant", "default", "multiple", [{ value: "default", label: "Default" }, { value: "long", label: "Long description" }, { value: "missing", label: "Missing optional links" }, { value: "multiple", label: "Multiple instructors" }]), PREVIEW_WIDTH_CONTROL, COLORWAY_CONTROL], render: ({ values }) => <CourseCardCollection courses={[{ ...sampleCourse, title: values.title, category: values.subject, description: values.variant === "long" ? longText : sampleCourse.description, instructors: values.variant === "multiple" ? [...sampleCourse.instructors, { name: "Guest Faculty", bio: "A second ordered instructor biography for the multi-instructor state." }] : sampleCourse.instructors, detailsUrl: values.variant === "missing" ? undefined : sampleCourse.detailsUrl, detailsLabel: values.variant === "missing" ? undefined : sampleCourse.detailsLabel, videoUrl: values.variant === "missing" ? undefined : sampleCourse.videoUrl, instructor: values.variant === "multiple" ? `${sampleCourse.instructor}, Guest Faculty` : sampleCourse.instructor }]} animateInitialCards={false} /> }),
