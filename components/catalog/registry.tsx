...
const COMPONENT_REGISTRY_BASE: ComponentRegistryEntry[] = [
  ...
  defineSpecimen({
    ...
    render: ({ colorway, surface, values }) => { const href = values.destination === "email" ? "mailto:hello@fractal.nyc" : "https://fractal.nyc"; return <div className="flex items-start"><OutboundLink href={href} accessibleName={values.label} tone={getComponentSurfaceTone(colorway, surface)}>{values.label}</OutboundLink></div>; },
  }),
  defineSpecimen({
  ...
  defineSpecimen({
    ...
    render: ({ colorway, surface, values }) => { const href = values.destination === "email" ? `mailto:${values.label}` : "https://fractal.nyc"; const link = <OutboundLink href={href} variant="inline" tone={getComponentSurfaceTone(colorway, surface)}>{values.label}</OutboundLink>; return values.context === "lead" ? <p className="text-body-lead" data-text-link-context="lead">First time here? Contact Crystal ({link}) for a guided tour.</p> : <p className="text-body" data-text-link-context="body">Email {link} if you want to teach at Fractal University.</p>; },
  }),
