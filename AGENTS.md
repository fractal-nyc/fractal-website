...
- **Banner SVG text uses embedded *subset* fonts:** the house banner SVGs (`public/images/banners/*-banner.svg`) render via `<img>`, so page-level fonts (the Google Fonts Jacquard 24 link) cannot reach them — only the `@font-face` data-URI *inside* each SVG applies. Each banner embeds a Jacquard 24 subset containing **only the letters that banner displays**. If you change banner text, re-subset the font (fontTools + `~/Library/Fonts/Jacquard24-Regular.ttf`) and splice the new base64 payload, or new letters silently render as missing glyphs (FRAC-32). Also: `<textPath>` clips text that overruns its arc path — check the arc is long enough for the new text (FRAC-31).
