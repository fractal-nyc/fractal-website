...
Browse specimens open in representative native site contexts so the gallery shows how Fractal's real house palette is used rather than flattening every example to monochrome. The focused view begins in that same context, keeps the large live preview first, and then shows only the options that component truly supports. Open **Usage details** for boundaries, current site usage, accessibility notes, technical source, and the same agent prompt. **Site color** and **Background** can still preview other approved token-backed combinations.
